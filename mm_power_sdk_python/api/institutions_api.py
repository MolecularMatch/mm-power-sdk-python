...
class InstitutionsApi(object):
    ...
    def get_private_trials(self, id, **kwargs):  # noqa: E501
        """Get a paginated list of private   # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.get_private_trials(id, async_req=True)
        >>> result = thread.get()
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.get_private_trials_with_http_info(id, **kwargs)  # noqa: E501
        else:
            (data) = self.get_private_trials_with_http_info(id, **kwargs)  # noqa: E501
            return data

    def get_private_trials_with_http_info(self, id, **kwargs):  # noqa: E501
        """Get a paginated list of private   # noqa: E501

        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.get_private_trials_with_http_info(id, async_req=True)
        >>> result = thread.get()
        ...
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_private_trials" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'id' is set
        if ('id' not in params or
                params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `get_private_trials`")  # noqa: E501
